...
async function main() {
  ...
  const depositTransaction = await depositProxyContract.connect(otherAccount).deposit(
  ...
  console.log(`Proxy deposit completed with transaction ${depositTransaction.hash}.`);
}
