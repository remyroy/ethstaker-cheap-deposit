...
/*describe("deposit_proxy", function () {
  it("Should return the correct deposit contract address", async function () {
    const depositContractAddress = '0xff50ed3d0ec03aC01D4C79aAd74928BFF48a7b2b';

    // deploy a lock contract where funds can be withdrawn
    // one year in the future
    const DepositProxy = await hre.ethers.getContractFactory("DepositProxyContract");
    const deposit_proxy = await DepositProxy.deploy(depositContractAddress);

    // assert that the value is correct
    expect(await deposit_proxy.depositContract()).to.equal(depositContractAddress);
  });
});*/